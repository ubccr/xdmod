...
// If $diff is 0, then all required role-related parameters are supplied
    ...
    if (count($diff) == 0) {
        ...
        if (empty($role_config['mainRoles'])) {
            $role_config['mainRoles'] = array();
        }
       
        if (isset($role_config['mainRoles'])) {
            $user_to_update->setRoles($role_config['mainRoles']);
        }
    
        if (isset($role_config['primaryRole'])) {
            $user_to_update->setPrimaryRole($role_config['primaryRole']);
        }
     ...
     // -----------------------------
        ...
        if (($activeRoleConfig['main_role'] == ROLE_ID_CENTER_DIRECTOR) || ($activeRoleConfig['main_role'] == ROLE_ID_CENTER_STAFF)) {
            if ($activeRoleConfig['main_role'] == ROLE_ID_CENTER_DIRECTOR) {
                $site_ref = 'centerDirectorSites';
                $active_center_ref = 'active_cd_center';
            }
            if ($activeRoleConfig['main_role'] == ROLE_ID_CENTER_STAFF) {
                $site_ref = 'centerStaffSites';
                $active_center_ref = 'active_cs_center';
            }
     ...
     // -----------------------------
        ...
        if (!in_array($user_to_update->getActiveRoleID(), $role_config['mainRoles'])) {
            ...
            if ($role_config['primaryRole'] == ROLE_ID_CENTER_DIRECTOR) {
                $organization_param = $role_config['primaryCenterDirectorSite'];
            }
            if ($role_config['primaryRole'] == ROLE_ID_CENTER_STAFF) {
                $organization_param = $role_config['primaryCenterStaffSite'];
            }
