    ...
    chartContextMenu: function(event, newchart, instance) {
        ...
        if (newchart) {
            ...
            if (CCR.xdmod.ui.isDeveloper) {
                ...
                /*generatePythonCode = function (config) {
                    let duration;
...
if (len(columns_list) > 10):
    ...
    for col in data_${i}.columns:
        data_0.insert(0,col,data_${i}[col])\n`
    }
    plotChart = 
    `${dataView}
...
with dw:`;
                ...
                const generatePythonCodeJson = function (config) {
                    let retJson = {
                        "metadata": {
                            "kernel_info": {
                                "name": "Python 3"
...
if (len(columns_list) > 10):
    ...
    label_${i} = dw.describe_metrics('${realm}').loc['${metric}', 'label']`
        ...
        multiData[i] += `
    for col in data_${i}.columns:
        data_0.insert(0,'${realm}: ' + 'Screwdriver' + ' [' + label_${i} + ']',data_${i}[col])\n` :
        multiData[i] += `
    for col in data_${i}.columns:
        data_0.insert(0,'${realm}: ' + col + ' [' + label_${i} + ']',data_${i}[col])\n`
    plotChart = 
    `${dataView}
    ${(swapXY && graphType !== 'pie') ? `\tdata_0 = data_0.reset_index()` : ''}
# Format and draw the graph to the screen\n
...
with dw:`;
                ...
                return JSON.stringify(retJson, null, 2);
                };*/
                ...
                /*menu.add({
                    text: 'Jupyter NB Code',
                    iconCls: 'chart',
                    handler: () => {
                        let ipynbFileRaw = generatePythonCodeJson(instance.getConfig())
