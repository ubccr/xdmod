...
# This script is used to export your saved Metric Explorer charts to image
# files using the XDMoD REST API.
#
# An XDMoD username and password are required to run the script.
#
# Before running the script,
#
# 1. Install the required `python-dotenv` and `requests` Python dependencies
#    (e.g., using `pip`).
# 2. Create a `.env` file in the same directory as the script that contains the
#    following contents, replacing `<username>` with your XDMoD username and
#    `<password>` with your XDMoD password — make sure to secure this file with
#    read-only access:
#        XDMOD_USERNAME=<username>
#        XDMOD_PASSWORD=<password>
# 3. Update the value of `site_address` at the top of the script with the URL
#    associated with your XDMoD portal.
# 4. Update the value of `export_dir` at the top of the script with the desired
#    directory path where the images will be written.
# 5. Confirm the desired values for `image_format`, `width`, and `height` at
#    the top of the script. The default image format is `svg`, but `png` and
#    `pdf` formats are also supported.
#
# By default, the script will download all of your saved Metric Explorer
# charts. You can have it instead download a single chart by providing the `-n`
# or `--name` option followed by the name of the saved chart.
