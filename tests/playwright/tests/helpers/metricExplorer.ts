export default {
    ...
    metricContextEntryByName: async function (name) {
    ...
    contextMenuItemByText: async function (text) {
        return '//div[@class="x-menu x-menu-floating x-layer x-menu-nosep"]//span[contains(text(), "' + text + '")]';
    },
    resizableWindowByTitle: async function (title) {
    ...
    rawDataEntryByIndex: async function (index) {
        var thisIndex = (index !== undefined) ? index : 1;
    ...
    load: {
        button: async function meLoadButtonId() {
        ...
        chartNum: async function meChartByIndex(number) {
            var thisNumber = number + 1;
            return '.x-menu-floating:not(.x-hide-offsets) .x-grid3-body > div:nth-child(' + thisNumber + ')';
        }
    },
    addData: {
    ...
    deleteChart: '',
    undo: async function meUndoButtonId() {
        // eslint-disable-next-line no-undef
        return '#' + this.container('button.x-btn-text-icon')
            .closest('table')
            .attr('id');
    },
    options: {
