...
class SEMAverageWaitHoursStatistic extends \DataWarehouse\Query\Jobs\Statistic
{
    ...
    {
        parent::__construct('coalesce(sqrt((sum(coalesce(jf.sum_waitduration_squared,0))/sum(jf.started_job_count))-pow(sum(coalesce(jf.waitduration,0))/sum(jf.started_job_count),2))/sqrt(sum(jf.started_job_count)),0)/3600.0', 'sem_avg_waitduration_hours', 'Std Dev: Wait Hours: Per Job', 'Hour', 2);
