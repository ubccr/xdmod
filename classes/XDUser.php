...
class XDUser
{
   ...
   */
    ...
    private function _isValidOrganizationID($role_id, $organization_id)
    {
        ...
        $results = $this->_pdo->query(
            "SELECT COUNT(*) AS num_matches FROM UserRoleParameters WHERE user_id=:user_id AND role_id=:role_id AND param_value=:organization_id",
            array(
            'user_id' => $this->_id,
            'role_id' => $role_id,
   ...
   */
    ...
    public function getRoleCategories()
    {
      ...
      order by short_name"
        ...
        if (count($rps) > 1) {
            foreach ($rps as $rp) {
                if (isset($role_parameters['provider']) && $role_parameters['provider'] == $rp['id']) {
                    continue; //ignore the rp that is linked to the role
                }
        ...
        */
        ...
        foreach ($roleCategories as $k => $v) {
            ...
            } elseif (!in_array($resourceProviderMetaData[1], $center_ids)) {
                $filteredRoleCategories[$k] = $v;
            }
        }//foreach ($roleCategories as $k => $v)
