...
class CloudStateReconstructorTransformIngestor extends pdoIngestor implements iAction
{
    ...
    public const REQUEST_START = 1;
    public const REQUEST_RESUME = 7;
    public const UNPAUSE_END = 57;
    public const UNPAUSE_START = 56;
    public const POWER_ON_START = 58;
    public const UNSUSPEND_START = 60;
    public const UNSHELVE_END = 63;
    public const START = 2;
    public const RESUME = 8;
    public const STATE_REPORT = 16;
    public const UNSHELVE = 20;
    public const UNSUSPEND = 61;
    public const POWER_ON = 59;
    public const STOP = 4;
    public const REQUEST_STOP = 3;
    public const REQUEST_TERMINATE = 5;
    public const POWER_OFF_START = 44;
    public const PAUSE_START = 54;
    public const SUSPEND_START = 62;
    public const SHELVE_START = 64;
    public const TERMINATE = 6;
    public const SUSPEND = 17;
    public const SHELVE = 19;
    public const POWER_OFF = 45;
    public const PAUSE = 55;
    public const RESIZE = 49;
    ...
    public function __construct(aOptions $options, EtlConfiguration $etlConfig, LoggerInterface $logger = null)
    {
        ...
        $this->_state_change_events = [self::REQUEST_START, self::START, self::REQUEST_STOP, self::STOP, self::TERMINATE, self::REQUEST_TERMINATE, self::REQUEST_RESUME, self::RESUME, self::SUSPEND, self::SHELVE, self::UNSHELVE, self::POWER_OFF_START, self::POWER_OFF, self::PAUSE_START, self::PAUSE, self::UNPAUSE_START, self::UNPAUSE_END, self::POWER_ON_START, self::POWER_ON, self::UNSUSPEND_START, self::UNSUSPEND, self::SUSPEND_START, self::UNSHELVE_END, self::SHELVE_START, self::STATE_REPORT, self::RESIZE];

        $this->_vm_inactive_events = [self::STOP, self::SUSPEND, self::SHELVE, self::POWER_OFF, self::PAUSE];
    ...
    private function initInstance($srcRecord): void
    {
        ...
        if ($srcRecord['event_type_id'] != self::TERMINATE) {
            $default_end_time = $this->_end_time ?? $srcRecord['event_time_ts'];

            $this->_instance_state = ['resource_id' => $srcRecord['resource_id'], 'instance_id' => $srcRecord['instance_id'], 'start_time_ts' => $srcRecord['event_time_ts'], 'start_event_id' => $srcRecord['event_type_id'], 'end_time_ts' => $default_end_time, 'end_event_id' => self::STOP];
        }
    }
