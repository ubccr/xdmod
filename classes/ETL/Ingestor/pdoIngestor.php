...
class pdoIngestor extends aIngestor
{
    ...
    private function singleDatabaseIngest()
    {
        ...
        $result = $this->destinationHandle->query("SHOW WARNINGS");

        if ( 0 != count($result) ) {
            $this->logger->warning(sprintf("SQL warnings on table '%s' generated by action %s", $qualifiedDestTableName, $this->name));
            foreach ( $result as $row ) {
                $this->logger->warning(implode(' ', $row));
            }
        }
                ...
                if ( $numRecordsInFile == self::MAX_RECORDS_PER_INFILE ) {
                    ...
                    foreach ( $loadStatementList as $etlTableKey => $loadStatement ) {
                        try {
                            ...
                            if ( count($output) > 0 ) {
                                $this->logger->warning(sprintf("LOAD DATA warnings on table '%s' generated by action %s", $etlTableKey, $this->name));
