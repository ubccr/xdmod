...
class Packager
{
    ...
    public static function createFromModuleName($module)
    {
        ...
        return new static(['module'       => $module, 'src_dir'      => $srcDir, 'module_dir'   => $moduleDir, 'config'       => $config, 'dest_dir'     => $destDir, 'package_name' => $packageName, 'logger'       => $logger]);
    }
     ...
     */
    public function setGitClone($clone): void
    {
        $this->gitClone = $clone;
    }
     ...
     */
    public function setGitBranch($branch): void
    {
     ...
     */
    public function setRunTests($runTests): void
    {
     ...
     */
    public function setExtractAssets($extractAssets): void
    {
     ...
     * repository.
     */
    private function cloneSourceRepo(): void
    {
        $clonePath = $this->getTmpDir() . '/xdmod-clone';

        // Clone source code from git.
        $gitArgs = ['git', 'clone', '--branch', $this->gitBranch, '--depth', '1', 'file://' . $this->srcDir, $clonePath];
     ...
     * Extract assets.
     */
    private function executeAssetsSetup(): void
    {
     ...
     * Run module tests.
     */
    private function runModuleTests(): void
    {
     ...
     */
    private function runCommandsPreBuild(): void
    {
    ...
    private function createModuleFile(): void
    {
        ...
        $data = ['#' => '*** WARNING: THIS FILE IS AUTOGENERATED. MODIFY AT YOUR OWN RISK ***', $name => ["display" => ucfirst($name), "enabled" => true, "packaged_on" => date("Y-m-d H:i:s"), "version" => ["major" => $this->config->getVersionMajor(), "minor" => $this->config->getVersionMinor(), "patch" => $this->config->getVersionPatch(), "pre_release" => $this->config->getVersionPreRelease(), "value" => $this->config->getVersion()]]];
        $modulesDirectory = implode(
            DIRECTORY_SEPARATOR,
            [$this->getPackageDir(), "configuration", "modules.d"]
        );
    ...
    private function processRpmSpec(): void
    {
    ...
    private function copyModuleFiles(): void
    {
        ...
        $files = ['AUTHORS.md', 'CHANGELOG.md', 'LICENSE', 'README.md'];
        ...
        // All optional.
        $dirs = array_diff(scandir($srcDir), ['.', '..', '.git', '.github', 'assets', 'docs']);
    ...
    private function createInstallScript(): void
    {
        ...
        $template->apply(['name'      => $this->config->getName(), 'version'   => $this->config->getVersion(), 'file_maps' => $fileMaps]);
     ...
     * Create a tar file.
     */
    private function createTarFile(): void
    {
     ...
     */
    private function copyFile($srcFile, $destFile): void
    {
     ...
     */
    private function copyDir($srcDir, $destDir): void
    {
