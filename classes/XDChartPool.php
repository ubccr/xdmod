	...
	class XDChartPool {
		...
		public function __construct(private $_user) {
         ...
         $this->_pdo = DB::factory('database');
			...
			$this->_person_id = $_user->getPersonID();
			$this->_user_full_name = $_user->getFormalName();
			$this->_user_token = $_user->getToken();
      ...
      public function emptyCache(): void {
		...
		public function addChartToQueue($chartIdentifier, $chartTitle, $chartDrillDetails, $chartDateDesc): void {
         ...
         $this->_pdo->execute(
            $insertQuery, 
            ['user_id' => $this->_user_id, 'chart_id' => $chartIdentifier, 'chart_title'=> $chartTitle, 'chart_date_description' => $chartDateDesc, 'chart_drill_details'=> $chartDrillDetails]
         );
		...
		public function removeChartFromQueue($chartIdentifier): void {
         ...
         $this->_pdo->execute("DELETE FROM {$this->_table_name} WHERE user_id = :user_id AND chart_id = :chart_id", ['user_id' => $this->_user_id, 'chart_id' => $chartIdentifier]);
		...
		public function chartExistsInQueue($chartIdentifier, $chartTitle = '') {
         ...
         if (empty($chartIdentifier)){
			...
			$results = $this->_pdo->query("SELECT * FROM {$this->_table_name} WHERE user_id = :user_id AND chart_id = :chart_id", ['user_id' => $this->_user_id, 'chart_id' => $chartIdentifier]);
