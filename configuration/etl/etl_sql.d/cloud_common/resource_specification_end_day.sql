...
FROM
    ...
    FROM
        ...
        LEFT JOIN
            (SELECT r.resource_id, r.host_id, MIN(r.fact_date) AS min_fact_date FROM modw_cloud.staging_resource_specifications AS r GROUP BY r.resource_id, r.host_id) AS rs2 ON rs.resource_id = rs2.resource_id
        GROUP BY
            rs.resource_id,
            rs.fact_date,
            rs2.host_id
        HAVING
            MIN(rs2.min_fact_date) <= rs.fact_date) AS rss1
    LEFT JOIN
...
GROUP BY
    rs.resource_id, rs.host_id, rs.fact_date, rs.memory_mb, rs.vcpus
//
