-- In order to change from a multi column to single column unique auto increment id field
-- we change the id field on the table to not be auto increment, drop the auto increment
-- key and then drop the id field. When ingestion is run at the end of the migration
...
LOCK TABLES
  ...
  modw_cloud.instance AS i WRITE,
  modw_cloud.instance_type WRITE,
  modw_cloud.instance_type AS it WRITE,
...
DROP INDEX instance_type_resource_idx ON modw_cloud.instance_type;
