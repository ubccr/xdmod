...
function handle_uncaught_exception($exception)
{
    ...
    $logger->error(array( 'message' => 'Exception Code: '.$exception->getCode()));
    $logger->error(array( 'message' => 'Message: '.$exception->getMessage()));
    $logger->error(array( 'message' => 'Origin: '.$exception->getFile().' (line '.$exception->getLine().')'));

    $stringTrace = (get_class($exception) == 'UniqueException') ? $exception->getVerboseTrace() : $exception->getTraceAsString();

    $logger->error(array('message' => "Trace:\n".$stringTrace."\n-------------------------------------------------------"));
