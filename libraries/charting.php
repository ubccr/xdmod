...
function processForReport(&$highchart_config): void
{
...
function processForThumbnail(&$highchart_config): void
{
    if ( isset($highchart_config['data']) && isset($highchart_config['data'][0]) ) {
        $highchart_config = $highchart_config['data'][0];
    }
...
function exportHighchart(
    ...
    $globalChartOptions = ['timezone' => date_default_timezone_get()];
    ...
    return match ($format) {
        'png' => convertSvg($svg, 'png', $effectiveWidth, $effectiveHeight, $fileMetadata),
        'pdf' => convertSvg($svg, 'pdf', round($width / 90.0 * 72.0), round($height / 90.0 * 72.0), $fileMetadata),
        default => $svg,
    };
}
